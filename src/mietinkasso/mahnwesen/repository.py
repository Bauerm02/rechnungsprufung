...
class MahnFallRepository:
    ...
    def letzter_mahnfall(self, vertrag_id: str) -> MahnFallTable | None:
        with self._session_factory() as session:
            statement = (
    ...
    def letzter_mahnfall_je_stufe(self, vertrag_id: str, stufe: int) -> MahnFallTable | None:
        with self._session_factory() as session:
            statement = (
                select(MahnFallTable)
                .where(MahnFallTable.vertrag_id == vertrag_id)
    ...
    def claim_fuer_versand(self, mahnfall_id: int) -> bool:
        """Atomarer Compare-and-Swap: nur der Worker, dessen UPDATE eine Zeile
        trifft, darf tatsächlich versenden. Ein zweiter Worker (oder ein
        Neustart, der denselben Fall erneut anstößt) sieht rowcount==0 und
        bricht ab, statt doppelt zu versenden."""
        ...
        with self._session_factory() as session:
            result = session.execute(
                ...
                .values(letzter_versuch_am=datetime.now(timezone.utc))
            )
            session.commit()
