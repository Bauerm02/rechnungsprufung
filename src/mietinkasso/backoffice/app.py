...
def indexautomatik_monatsbericht_liste(request: Request, session=Depends(_current_session)) -> HTMLResponse:
    berichte = _indexautomatik.monatsbericht_repository.liste_alle()
    zeilen = "".join(
    ...
    <div class="card">
      ...
      <p class="status-zeile">
        <span>Owner-Mailversand (Monatsbericht):
          <strong>{"aktiviert" if _settings.index_monatsbericht_send_enabled else "deaktiviert"}</strong>,
...
def indexautomatik_monatsbericht_detail(request: Request, periode: str, session=Depends(_current_session)) -> HTMLResponse:
    bericht = _indexautomatik.monatsbericht_repository.get_by_periode(periode)
    if bericht is None:
        return _fehlerseite(
